...
CREATE TABLE "book" (
	...
	('coupon_type1','coupon_type2','coupon_type3','coupon_type4','coupon_type5');

	--クーポンテーブルの作成
	CREATE TABLE coupon (
    id SERIAL PRIMARY KEY,                  -- ID
    user_name VARCHAR(255) NOT NULL,       -- ユーザーネーム
    coupon_type coupon_type NOT NULL,    -- クーポンの種類
    FOREIGN KEY (user_name) REFERENCES "user"(username)  -- 外部キー制約
);
